...
export const listCompanies = async (req, res) => {
  try {
    // Some deployments may not have an `archived` column. Fetch all and filter in JS
    const { data, error } = await supabase
      .from('companies')
      .select('*');
    ...
    const visible = (data || []).filter(c => !c.archived);

    const companiesWithDefaults = visible.map(company => ({
      ...company,
