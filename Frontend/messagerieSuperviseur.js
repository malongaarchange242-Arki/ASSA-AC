document.addEventListener('DOMContentLoaded', async () => {
    ...
    // -----------------------
    function getToken() {
        return localStorage.getItem('jwtTokenSuperviseur') || localStorage.getItem('jwtTokenAdmin') || localStorage.getItem('jwtTokenCompany');
    }
    ...
    function getSessionInfo() {
        ...
        try {
            ...
            const rawRole = (payload.role || payload.profile || '').toLowerCase().trim();
            const isSupervisor = ['superviseur', 'superviseur', 'daf'].includes(rawRole);
            const isAdmin = ['administrateur', 'super_directeur', 'admin'].includes(rawRole);
            return {
                token,
                ...
                role: isSupervisor || isAdmin ? 'admin' : 'company'
            };
    ...
    async function fetchWithAuth(url, options = {}) {
        ...
        if (newToken) {
            // Déterminer le bon clé localStorage basé sur le token original
            let tokenKey = 'jwtTokenSuperviseur';
            try {
                const payload = JSON.parse(atob(session.token.split('.')[1] || '{}'));
                const rawRole = (payload.role || '').toLowerCase().trim();
                if (rawRole.includes('admin') || rawRole === 'administrateur' || rawRole === 'super_directeur') {
                    tokenKey = 'jwtTokenAdmin';
                } else if (rawRole === 'company') {
                    tokenKey = 'jwtTokenCompany';
                }
            } catch (e) { /* fallback to jwtTokenSuperviseur */ }
    ...
    async function fetchCompaniesActive() {
        try {
            ...
            if (!session) { 
                ...
                companyContainer.innerHTML = '<p style="color:red;">Session expirée.</p>'; 
                return; 
            }

            console.log('Fetching companies from:', `${API_BASE}/api/companies/all`);
            ...
            if (!activeCompanies.length) { 
                companyContainer.innerHTML = '<p style="padding: 16px; text-align: center; color: var(--muted);">Aucune compagnie active trouvée.</p>'; 
                return; 
            }
            ...
            companyContainer.innerHTML = `<p style="padding: 16px; color: red; font-size: 12px;">Erreur: ${err.message}</p>`;
        }
    }
    ...
    // 3. Écouteur du bouton de déconnexion
    if (logoutBtn) {
        logoutBtn.addEventListener('click', () => {
            localStorage.removeItem('jwtTokenSuperviseur');
            localStorage.removeItem('jwtTokenAdmin');
            localStorage.removeItem('jwtTokenCompany');
            localStorage.removeItem('adminId');
            localStorage.removeItem('id_companie');
            localStorage.removeItem('refreshTokenSuperviseur');
