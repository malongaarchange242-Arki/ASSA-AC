...
function renderMessage(message, attachments){
    ...
    const isCompany = role !== 'admin';

    const wrapper = document.createElement('div');
    wrapper.className = isCompany ? 'flex justify-start' : 'flex justify-end';
    wrapper.setAttribute('data-id', message.id);

    const bubble = document.createElement('div');
    // Styles selon l'expéditeur
    if(isCompany){
        bubble.className = 'p-3 rounded-xl rounded-bl-none bg-gray-300 text-gray-900 max-w-xs shadow';
    } else if(isAdmin){
        bubble.className = 'p-3 rounded-xl rounded-br-none bg-blue-500 text-white max-w-xs shadow';
    } else {
