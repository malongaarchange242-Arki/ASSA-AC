document.addEventListener('DOMContentLoaded', () => {
    ...
    async function verifyEmailOnServer(email) {
        ...
        try {
            ...
            if (data.role === 'admin' || data.role === 'supervisor' || data.role === 'superviseur') {
            ...
            } else if (data.role === 'operateur' || data.role === 'operator') {
                // Opérateur : utilise l'auth avec mot de passe (similaire à Admin)
                passwordField.classList.add('visible');
                actionButtonContainer.classList.add('visible');
                mainButton.textContent = 'Se Connecter';
                mainTitle.textContent = 'Mot de passe';
                subTitle.textContent = `Veuillez entrer votre mot de passe, opérateur.`;
                passwordInput.focus();
            } else {
    ...
    try {
        ...
        if (uiRole === 'admin') {
            ...
            isCompany = true;
        } else if (uiRole === 'operateur' || uiRole === 'operator') {
            url = `${API_BASE}/api/operateurs/login`;
        } else {
            alert('Rôle inconnu.');
            return;
        }

        console.log('📡 Tentative login', { email: currentEmail, role: uiRole, url });
        ...
        if (!res.ok) {
            console.error('❌ Response failed:', res.status, data);
        ...
        const accessToken = data.token || data.jwtTokenAdmin || data.accessToken || data.access_token;
        const refreshToken = data.refreshToken || data.refreshTokenAdmin || data.refresh_token;

        if (!accessToken) throw new Error('Token manquant dans la réponse serveur.');

        // 🔥 Déterminer le rôle côté backend
        const rawRole = data.role || data.profile || data.profileType || uiRole;
        const role = rawRole.toString().toLowerCase().trim().replace('é', 'e');
        ...
        // Store role-specific tokens for operateur and superviseur
        if (role === 'operateur' || role === 'operator') {
            localStorage.setItem('jwtTokenOperateur', accessToken);
            localStorage.setItem('userRoleOperateur', role);
            localStorage.setItem('userEmailOperateur', currentEmail);
            if (data.id) localStorage.setItem('operateurId', data.id);
        }
        if (role === 'supervisor' || role === 'superviseur') {
            localStorage.setItem('jwtTokenSuperviseur', accessToken);
            localStorage.setItem('userRoleSuperviseur', role);
            localStorage.setItem('userEmailSuperviseur', currentEmail);
            if (data.id) localStorage.setItem('superviseurId', data.id);
        }
        ...
        alert(`Erreur login : ${err.message || 'Serveur indisponible'}`);
    }
}
