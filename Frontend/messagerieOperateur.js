document.addEventListener('DOMContentLoaded', async () => {
    ...
    // -----------------------
    function getToken() {
        return localStorage.getItem('jwtTokenOperateur');
    }
    ...
    function getSessionInfo() {
        ...
        try {
            ...
            const rawRole = (payload.role || payload.profile || '').toString().toLowerCase();
            const isAdmin = ['administrateur', 'superviseur', 'super admin', 'admin'].includes(rawRole);
            const isOperateur = rawRole.includes('operat') || rawRole === 'operateur' || rawRole === 'operator';
            return {
                token,
                ...
                role: isAdmin ? 'admin' : (isOperateur ? 'operateur' : 'company'),
                operateurId: payload.id || localStorage.getItem('operateurId')
            };
    ...
    async function fetchWithAuth(url, options = {}) {
        ...
        if (newToken) {
            // persist refreshed token for operator only
            localStorage.setItem('jwtTokenOperateur', newToken);
