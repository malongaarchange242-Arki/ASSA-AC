...
function getAuthToken() {
    ...
    try {
        ...
        console.error("Token opérateur invalide:", err.message);
...
async function loadClients() {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/companies/all`, {
                headers: { 'Authorization': `Bearer ${token}`, 'X-Operateur-Id': localStorage.getItem('operateurId') || '' }
        });
...
async function fetchNextInvoiceId() {
    ...
    try {
        ...
        const response = await fetch(url, {
            method: "GET",
                headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json", 'X-Operateur-Id': localStorage.getItem('operateurId') || '' }
        });
...
async function showPreview() {
    ...
    try {
        // Ensure invoice reference exists; try to fetch if missing (operator flow)
        if (!invoiceIdInput.value) {
            await fetchNextInvoiceId();
            if (!invoiceIdInput.value) {
                showMessage("Impossible d'obtenir la référence de facture. Vérifiez la période et la compagnie.", 'error');
                return;
            }
        }
