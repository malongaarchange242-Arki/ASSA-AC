...
async function loadCompanyInvoices() {
    try {
        ...
        try {
            ...
            base = 'https://assa-ac-duzn.onrender.com';
        ...
        // Mapping factures pour l'UI (normalise les noms de champs utilisés ailleurs)
        SERVER_INVOICES = data.map(f => ({
            id: f.id,
            numero_facture: f.numero_facture || f.numero || f.ref || f.id,
            date: f.date || f.date_emission || f.created_at,
            amount: Number(f.amount || f.montant || f.total || 0),
            // status/ statut : on maintient les deux clés pour compatibilité
            status: f.status || f.statut || '',
            statut: f.status || f.statut || '',
            due_date: f.due_date,
            client: f.client || f.nom_client || ''
        }));
...
function renderInvoiceSelect(viewId, filterFn = inv => true) {
    ...
    targetInvoices.forEach(inv => {
        const ref = inv.numero_facture || inv.id;
        const amount = Number(inv.amount || 0).toLocaleString('fr-CM', { style: 'currency', currency: 'XAF' });
        const stat = inv.status || inv.statut || '-';
        options += `<option value="${ref}">${ref} (${amount} - Statut: ${stat})</option>`;
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (paiementForm) {
        paiementForm.addEventListener('submit', async e => {
            ...
            const invoiceSelectEl = document.getElementById('invoice-select-paiement');
            if (!invoiceSelectEl) {
                showModal('Erreur', 'Champ facture introuvable. Veuillez recharger la page.');
                return;
            }
            const invoiceId = invoiceSelectEl.value;

            const noteEl = document.getElementById('paiement-note');
            const note = noteEl ? (noteEl.value || '') : '';

            if (!fileInputPaiement) {
                showModal('Erreur', 'Champ fichier introuvable. Veuillez recharger la page.');
                return;
            }
