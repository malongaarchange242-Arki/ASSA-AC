...
function loadTable(data = requests) {
  ...
  tbody.innerHTML = data.map(row => {
    ...
    const pretty = formatAmountValue(displayInvoiceValue);
    // Build link to Facture_.html that will auto-download the invoice PDF
    ...
    const invHtml = displayInvoiceValue ? (
    ...
    const docAction = draftAvailable
      ...
      : `<a href="#" class="draft-download" data-request="${escapeHtml(String(row.request_id || row.id || ''))}" data-type="${isFeri ? 'FERI' : 'DRAFT'}" title="Aucun draft disponible"><i class="far fa-file-pdf text-muted" title="Document non disponible"></i></a>`;
    ...
    return `
      <tr>
        ...
        <td>${invHtml}</td>
        <td style="color: #64748b;">${escapeHtml(row.feri_ref || row.feriRef || row.ectn || '---')}</td>
        <td>${docAction}</td>
      </tr>
    `;
  }).join('');
...
function renderRow(r) {
  ...
  const pretty = formatAmountValue(invoice);
  ...
  const docAction = feriUrlRow
    ? `<a class="doc-download" href="${escapeHtml(feriUrlRow)}" target="_blank" rel="noopener noreferrer"><i class="far fa-file-pdf" style="color: #78B13F; cursor: pointer;"></i> ${escapeHtml(availabilityLabelRow)}</a>`
    : `<i class="far fa-file-pdf text-muted" title="Document non disponible"></i>`;
    ...
    tableBody.addEventListener('click', function (e) {
      ...
      if (draftLink) {
        ...
        if (!draftLink.dataset.shown) {
          ...
          // try to resolve request type from cached requests to choose correct label
          const rid = draftLink.getAttribute('data-request');
          let reqTypeLabel = '';
          try {
            const reqObj = requests.find(r => String(r.request_id || r.id || '') === String(rid));
          ...
          // restore icon after 6s for clarity — preserve whether icon was muted
          ...
          setTimeout(() => {
            if (draftLink && draftLink.dataset) {
              ...
              // restore original icon (keep same HTML structure used in renderRow)
...
function goToStep2(type) {
  ...
  if (title) title.innerText = (type === 'FERI') ? "saisir pour Numero FXI" : "Issue a CTN " + type;
