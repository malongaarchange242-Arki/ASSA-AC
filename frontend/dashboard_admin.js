...
document.getElementById('lang-select').addEventListener('change', () => {
  applyTranslations(); // Reapply translations when language is switched
});
...
function renderAdminTable() {
  ...
  tbody.innerHTML = rowsToShow.map(row => {
    ...
    return `
      ...
      ? `<span class="pending-payment" style="color:#f59e0b; font-weight:600;">Pending Payment</span>`
      : (isInitiated ?
        ...
        (String(status) === 'PAYMENT_PROOF_UPLOADED'
          ? `<span class="pending-confirmation" style="color:#f59e0b; font-weight:600;">Pending Confirmation</span>`
...
async function handleAdminSubmit() {
  ...
  try {
    ...
    if (currentMode === 'FINAL') {
      // Keep existing final publish flow
      const fd = new FormData();
      fd.append('feri_ref', inputValue);
      if (fileInp && fileInp.files && fileInp.files[0]) fd.append('file', fileInp.files[0]);
...
function openSidePanel(req) {
  ...
  if (!panel) return;
  document.getElementById('side-bl').innerText = req.extracted_bl || req.bl || req.bl_number || '—';
