...
export class AdminService {
  ...
  static async generateFinalDocument(requestId: string, adminId: string, kind: 'FERI' | 'AD') {
    ...
    const pythonUrlBase = process.env.PYTHON_SERVICE_URL || 'https://mkc-5slv.onrender.com';
  ...
  static async sendDraft(requestId: string, adminId: string, opts: { amount?: number | null; currency?: string; file?: Express.Multer.File; cargo_route?: string } = {}) {
    try {
      ...
      if (invoiceErr || !invoiceData) throw new Error('Failed to create or update invoice');
      ...
      try {
        ...
        await NotificationsService.send({
          ...
          entityId: requestId,
          channels: ['in_app', 'email'],
          links: [{ name: 'Proforma', url: signed, expires_in: 60 * 60 * 24 * 3 }],
